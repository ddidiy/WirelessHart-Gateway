 ...
 * Also can be called more than once (second time will reset the stream)
 */
...
#define STREAM_READ_PACKED(stream, bytes, length)\
	{\
		...
		uint8_t len = (uint8_t)(length * 6 /8) ;\
/* doinel.alban		uint8_t str[len];*/\
		uint8_t str[(uint8_t)(length * 6 /8)];\
		memcpy(str, (stream)->nextByte, len);\
		uint8_t pack;\
		for (uint8_t i=0, j=0; i+3<len; i=i+3, j=j+4) {\
			if(str[i]=='\0'){\
				bytes[j]='\0';\
				break;\
			}\
...
#define STREAM_WRITE_PACKED(stream, bytes, length)\
	{\
		...
		uint8_t ch[3]={'?','?','?'};\
		STREAM_SET_ERROR_ON_COND(stream,BinaryStream_Err_TooFewBytes, (length%4)!=0);\
		for (uint8_t i=0;i+4<length;i=i+4) {\
