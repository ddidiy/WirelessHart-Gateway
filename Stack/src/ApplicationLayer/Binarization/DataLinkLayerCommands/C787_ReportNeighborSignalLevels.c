...
{
  context->cmdId = CMDID_C787_ReportNeighborSignalLevels;
...
{
#if defined(_DEBUG)
//   if (fromStream->remainingBytes > C787_ReqSize)
		                    ...
		                    BinaryStream* fromStream )
{
  ...
  if (fromStream->remainingBytes < response->m_ucNoOfNeighborEntriesRead * 3)
