...
uint8_t Compose_C077_SendCommandToSubDevice_Req(C077_SendCommandToSubDevice_Req* request, ComposerContext* context,
		BinaryStream* toStream)
{
	context->cmdId = CMDID_C077_SendCommandToSubDevice;
...
uint8_t Parse_C077_SendCommandToSubDevice_Req(C077_SendCommandToSubDevice_Req* request, ParserContext* context,
		BinaryStream* fromStream)
{
	...
	//check if enough bytes in stream for reading address + cmd of embedded cmd + embedded cmd size
	if ( fromStream->remainingBytes < ( (request->addressType == 0) ? ShortFieldAddressSize : LongFieldAddressSize ) + 2 )
	{
	...
	//check if enough bytes in stream for reading embedded command size
	if ( fromStream->remainingBytes <  request->embeddedCommandSize )
	{
		return RCS_E05_TooFewDataBytesReceived;
	}
...
uint8_t Compose_C077_SendCommandToSubDevice_Resp(C077_SendCommandToSubDevice_Resp* response, ComposerContext* context,
		BinaryStream* toStream)
{
	context->cmdId = CMDID_C077_SendCommandToSubDevice;
...
uint8_t Parse_C077_SendCommandToSubDevice_Resp(C077_SendCommandToSubDevice_Resp* response, ParserContext* context,
		BinaryStream* fromStream)
{
	...
	//check if enough bytes in stream for reading address + cmd of embedded cmd + embedded cmd size
	if ( fromStream->remainingBytes < ( (response->addressType == 0) ? ShortFieldAddressSize : LongFieldAddressSize ) + 2 )
	{
		return RCS_E05_TooFewDataBytesReceived;
	}
	...
	else if (response->addressType == 1)
	{
	...
	//check if enough bytes in stream for reading embedded command size
	if ( fromStream->remainingBytes <  response->embeddedCommandSize )
	{
		return RCS_E05_TooFewDataBytesReceived;
	}
