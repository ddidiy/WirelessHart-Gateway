...
{
	context->cmdId = CMDID_C012_ReadMessage;
		...
		ComposerContext* context, BinaryStream* toStream)
{
	...
	STREAM_WRITE_PACKED(toStream, response->message,sizeof(response->message));
		...
		ParserContext* context, BinaryStream* fromStream)
{
		...
		#if defined(_DEBUG)
// 			if (fromStream->remainingBytes < C012_RespSize)
// 				return RCS_E05_TooFewDataBytesReceived;
		#endif

		STREAM_READ_PACKED(fromStream, response->message, sizeof(response->message));
