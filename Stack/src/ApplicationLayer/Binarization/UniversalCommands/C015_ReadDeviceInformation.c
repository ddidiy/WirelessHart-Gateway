...
{
	context->cmdId = CMDID_C015_ReadDeviceInformation;
		...
		ParserContext* context, BinaryStream* fromStream)
{
	...
	#if defined(_DEBUG)
// 		if (fromStream->remainingBytes < C015_RespSize)
// 			return RCS_E05_TooFewDataBytesReceived;
	...
	uint8_t hardcoded = 0;
	STREAM_READ_INT8(fromStream, &hardcoded); //read harcoded value 250
	if (hardcoded != C015_Byte16_Reserved)
			return 1; //FIXME [george.petrehus] - hardcoede value.
