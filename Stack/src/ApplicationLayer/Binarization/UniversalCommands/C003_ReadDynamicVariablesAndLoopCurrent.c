...
{
  context->cmdId = CMDID_C003_ReadDynamicVariablesAndLoopCurrent;
...
{
#ifdef _DEBUG
//   if(toStream->remainingBytes < C003_RespSize)
                                                           ...
                                                           ParserContext* context, BinaryStream* fromStream)
{
  ...
  response->noOfVariables = (fromStream->remainingBytes - 4) / 5;   // 4 bytes for PV loop current, 5 bytes for each variable
