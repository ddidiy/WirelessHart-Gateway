...
{
  context->cmdId = CMDID_C801_DeleteService;
  STREAM_WRITE_UINT8(toStream, request->m_ucServiceId);
  STREAM_WRITE_UINT8(toStream, request->m_ucReason);

  if (request->m_ucReason == 0xFF)
  {
	  STREAM_WRITE_UINT16(toStream, request->m_peerNickname);
  }
                                      ...
                                      BinaryStream* fromStream )
{
  ...
  if (request->m_ucReason == 0xFF)
  {
	  STREAM_READ_UINT16(fromStream, &request->m_peerNickname);
  }
