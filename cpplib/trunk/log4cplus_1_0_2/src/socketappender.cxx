...
// Revision 1.1  2008/05/09 15:04:00  andrei.petrut
// first build for cygwin
//
...
bool SocketAppender::logConnectToServerFail = false;
...
log4cplus::SocketAppender::openSocket()
{
    if(!socket.isOpen()) {
        ...
        if (!socket.isOpen()) {
        	if (!logConnectToServerFail) {
				getLogLog().error(LOG4CPLUS_TEXT("SocketAppender::openSocket()- Cannot connect to server!"));
				logConnectToServerFail = true;
        	}
        } else {
            getLogLog().warn(LOG4CPLUS_TEXT("SocketAppender::openSocket()- Connect to server successfully!"));
			logConnectToServerFail = false;
        }
    }
}

const tstring endChunkMessage = "#...#";

std::size_t getMaxChunkMessageSize(const log4cplus::spi::InternalLoggingEvent& event,
        const log4cplus::tstring& serverName) {
    ...
    // buffer.appendString(endChunkMessage);
    ...
    std::size_t maxChunkMessageSize = buffer.getMaxSize() - buffer.getSize() - 10;

    std::size_t endChunkMessageLength;
#ifndef UNICODE
    endChunkMessageLength = endChunkMessage.length();
#else
    endChunkMessageLength = endChunkMessage.length() * 2;
#endif

    maxChunkMessageSize -= endChunkMessageLength;

    return maxChunkMessageSize;
}
...
log4cplus::SocketAppender::append(const spi::InternalLoggingEvent& event)
{
    ...
    std::size_t maxChunkMessageSize = getMaxChunkMessageSize(event, serverName);
//    std::cout << "maxChunkMessageSize=" << (int)maxChunkMessageSize << std::endl;

    std::size_t indexEventMessage = 0;
    while (indexEventMessage < event.getMessage().size()) {
...
//        std::cout << "SocketAppender - indexEventMessage=" << (int)indexEventMessage
//			<< ", buffer=" << event.getMessage().substr(indexEventMessage,
//		    		std::min(maxChunkMessageSize, event.getMessage().size() - indexEventMessage))
//		    << std::endl;
        ...
        indexEventMessage += maxChunkMessageSize;
    }
...
log4cplus::helpers::convertToBuffer(const log4cplus::spi::InternalLoggingEvent& event,
		std::size_t indexEventMessage,
		std::size_t maxChunkMessageSize,
        const log4cplus::tstring& serverName)
{
    SocketBuffer buffer(LOG4CPLUS_MAX_MESSAGE_SIZE - sizeof(unsigned int));
    ...
    // buffer.appendString(event.getMessage());
    tstring chunk;
    if (maxChunkMessageSize > event.getMessage().size() - indexEventMessage) {
    	chunk = event.getMessage().substr(indexEventMessage, event.getMessage().size() - indexEventMessage);
    } else {
    	chunk = event.getMessage().substr(indexEventMessage, maxChunkMessageSize);
        chunk += endChunkMessage;
    }
