...
void Connection::Open(const std::string& dbPath, int timeout)
{
	...
	if (srv != SQLITE_OK)
	{
		LOG_ERROR_APP("WARNING sqlite3_enable_shared_cache: FAILED with errorcode" << srv );
	}
	...
	//journal=off
	{
	    ...
	    if( ret == SQLITE_OK )
	    {
	        ...
	        if (!colValue)
	        {
				...
				LOG_ERROR_APP( "Unable to get pragma journal_mode value!");
	        }
			else
	        LOG_DEBUG_APP("setting PRAGMA main.journal_mode = OFF; with result(string)= " << (const char*)colValue);
    ...
    //synchronous=off
    {
	    ...
	    if( ret == SQLITE_OK )
	    {
	        ...
	        LOG_DEBUG_APP("setting PRAGMA synchronous = OFF; with result(int)= " << colValue);
	        sqlite3_finalize(sqlStmt);
	    }
	    else
	    {
	...
	LOG_DEBUG_APP ("sqlite3_threadsafe =" << (int)sqlite3_threadsafe() );

	LOG_DEBUG_APP("Open database:" << dbPath);
}
...
void Connection::LogIfLastError(int p_nResultCode, const char* p_szContext)
{
	...
	LOG_ERROR_APP("LogIfLastError : "<< p_szContext << " err=" << sqlite3_errmsg(database) << " res="<< (int)p_nResultCode);
}
...
{
	//LOG_DEBUG_APP("Executing query:" << query);
...
{
	LOG_DEBUG_APP("Executing query:" << p_szQuery);
...
int CSqliteStmtHelper::Prepare (const char* p_szSql)
{
	...
	{
		LOG_ERROR_APP("CSqliteStmtHelper::Prepare - no database connection");
		return SQLITE_ERROR;
	}
	Release();
	...
	if (res != SQLITE_OK)
	{
		LOG_ERROR_APP("CSqliteStmtHelper::Prepare " << sqlite3_errmsg(m_oConnection.GetRawDbObj()) << " res="<< (int)res);
		LOG_ERROR_APP("CSqliteStmtHelper::Prepare query=" << m_szSql);
...
int CSqliteStmtHelper::Step (bool p_bExec)
{
	if (!m_pStmt)
	{
		LOG_ERROR_APP("CSqliteStmtHelper::Step called without prepare first");
		return SQLITE_ERROR;
	}
	...
	{
		LOG_ERROR_APP("CSqliteStmtHelper::Step " << sqlite3_errmsg(m_oConnection.GetRawDbObj()) << " res="<< (int)res);
	}
...
int		CSqliteStmtHelper::Column_Check ( int p_nCol, int p_nColType)
{
	...
	{
		LOG_ERROR_APP("CSqliteStmtHelper::Column_Check colType="<< p_nColType <<" col=" << p_nCol << " >= " << numColumns);
		return SQLITE_ERROR;
	}
	...
	{
		LOG_ERROR_APP("CSqliteStmtHelper::Column_Check colType="<< p_nColType <<"!= "<< colType <<" col=" << p_nCol <<" query=" << m_szSql );
		return SQLITE_ERROR;
	}
...
int		CSqliteStmtHelper::Column_GetDateTime ( int p_nCol, nlib::DateTime* p_poDateTime )
{
	...
	{
		LOG_DEBUG_APP("Column_GetDateTime: NULL");
	...
	{
		LOG_ERROR_APP("Column_GetDateTime: invalid datetime=" << szDate <<"  expected YYYY-MM-DD HH:mm:ss");
		*p_poDateTime = nlib::DateTime();
		return SQLITE_ERROR;
	}

	LOG_DEBUG_APP("Column_GetDateTime: datetime="<<szDate);
